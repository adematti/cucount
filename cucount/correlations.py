...
def calculate_correlation(particles_one, particles_two, spin_one, spin_two, randoms_particles_one=None, randoms_particles_two=None,
    ...
    if errors != 'jackknife':
        if spin_one == 0 and spin_two == 0:
            ...
            if randoms_particles_one is not None and randoms_particles_two is not None:
                corr = __compute_count_count_correlation__(
                    particles_one,
                    randoms_particles_two,
                    particles_one.weights,
                    randoms_particles_two.weights,
                    battrs,
                )
            else:
                # Simple pair counts without Landy-Szalay
        ...
        # Generate jackknife region assignments
        ...
        if spin_one == 0 and spin_two == 0:
            ...
            if randoms_particles_one is None or randoms_particles_two is None:
                raise ValueError("Jackknife for spin=(0,0) requires randoms_particles_one and randoms_particles_two")

            assignments_randoms_one = __jackknife__.assign_jackknife_regions(randoms_particles_one, n_jackknife_regions, seed)
            assignments_randoms_two = __jackknife__.assign_jackknife_regions(randoms_particles_two, n_jackknife_regions, seed)
...
def __compute_count_count_correlation__(
    ...
    ran_weights: np.ndarray,
    battrs,
) -> np.ndarray:
    """Compute the count-count correlation using Landy-Szalay estimator."""
    DD = count2(pos_particles, pos_particles, battrs=battrs, spin1=0, spin2=0)
    DR = count2(pos_particles, ran_particles, battrs=battrs, spin1=0, spin2=0)
    RR = count2(ran_particles, ran_particles, battrs=battrs, spin1=0, spin2=0)

    DD_norm = np.sum(pos_weights) ** 2 - np.sum(pos_weights**2)
    DR_norm = np.sum(pos_weights) * np.sum(ran_weights)
    RR_norm = np.sum(ran_weights) ** 2 - np.sum(ran_weights**2)
